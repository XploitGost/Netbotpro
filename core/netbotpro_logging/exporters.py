...
def _svg_bar_chart(labels, values, width=520, height=180):
    ...
    for i, (lab, val) in enumerate(zip(labels, values)):
        ...
        y = (height - pad) - bh
        parts.append(
            f'<rect x="{x:.2f}" y="{y:.2f}" width="{bw:.2f}" height="{bh:.2f}" rx="4" fill="#22c55e" opacity="0.9"><title>{html.escape(str(lab))}: {val}</title></rect>'
        )
        lab_s = str(lab)
        if len(lab_s) > 10:
            lab_s = lab_s[:10] + "..."
        parts.append(
            f'<text x="{x + bw / 2:.2f}" y="{height - 10}" text-anchor="middle" font-size="10" fill="#94a3b8">{html.escape(lab_s)}</text>'
        )
    parts.append("</svg>")
    return "".join(parts)


def _write_full_html(
    save_path: str,
    ...
    proto_counts = (
        ...
        and not df_packets.empty
        and "proto" in df_packets.columns
        else {}
    )
    top_src = (
        df_packets["src"].fillna("-").astype(str).value_counts().head(8).to_dict()
        if df_packets is not None
        and not df_packets.empty
        and "src" in df_packets.columns
        else {}
    )
    top_dst = (
        df_packets["dst"].fillna("-").astype(str).value_counts().head(8).to_dict()
        if df_packets is not None
        and not df_packets.empty
        and "dst" in df_packets.columns
        else {}
    )
    attack_counts = (
        df_alerts["attack"]
        ...
        and not df_alerts.empty
        and "attack" in df_alerts.columns
        else {}
    )
    ...
    parts = [
        '<!doctype html><html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>NetBotPRO Report</title>',
        f'<style>{css}</style></head><body><div class="wrap">',
    ]
    parts.append(
        f'<div class="head"><div><h1>NetBotPRO - Full Report</h1><div class="meta">Generated: {html.escape(now)} | Source: {html.escape(source)}</div></div><div class="meta">{html.escape(os.path.basename(save_path))}</div></div>'
    )
    parts.append('<div class="grid">')
    parts.append(
        f'<div class="card"><div class="k">Packets</div><div class="v">{p_count}</div></div>'
    )
    parts.append(
        f'<div class="card"><div class="k">Alerts</div><div class="v" style="color:var(--danger)">{a_count}</div></div>'
    )
    parts.append(
        f'<div class="card"><div class="k">Top proto</div><div class="v">{html.escape(next(iter(proto_counts.keys()), "-"))}</div></div>'
    )
    parts.append(
        f'<div class="card"><div class="k">Top src / dst</div><div class="v" style="font-size:14px">{html.escape(next(iter(top_src.keys()), "-"))} -> {html.escape(next(iter(top_dst.keys()), "-"))}</div></div>'
    )
    parts.append("</div>")
    parts.append('<div class="charts">')
    parts.append(
        '<div class="card"><div class="k">Protocol distribution</div>'
        + (
            _svg_bar_chart(list(proto_counts.keys()), list(proto_counts.values()))
            or '<div class="meta">No data</div>'
        )
        + "</div>"
    )
    parts.append(
        '<div class="card"><div class="k">Alerts by type</div>'
        + (
            _svg_bar_chart(
                list(attack_counts.keys()), [int(v) for v in attack_counts.values()]
            )
            or '<div class="meta">No alerts</div>'
        )
        + "</div>"
    )
    parts.append("</div>")
    if df_alerts is not None and not df_alerts.empty:
        cols = [
            c
            for c in ["ts", "src", "dst", "proto", "dport", "attack", "score", "engine"]
            if c in df_alerts.columns
        ]
        parts.append(
            '<div class="sec"><h2>Alerts</h2>'
            + df_alerts[cols].to_html(index=False, escape=True)
            + "</div>"
        )
    if df_packets is not None and not df_packets.empty:
        ...
        parts.append(
            '<div class="sec"><h2>Packets</h2>'
            + df_packets[main_cols].to_html(index=False, escape=True)
            + "</div>"
        )
    parts.append(
        '<div class="meta" style="margin-top:18px">Generated by NetBotPRO logging exporters</div></div></body></html>'
    )
...
def export_session_zip(
    ...
    traceroute_rows: list[dict] | None = None,
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        meta = {
            ...
            "source": "session",
            "counts": {
                "packets": int(len(df_p)),
                "alerts": int(len(df_a)),
                "traceroute": int(len(df_t)),
            },
        }
...
def export_all_history_zip(save_path: str) -> str:
    ...
    with tempfile.TemporaryDirectory() as td:
        df_p = pd.read_sql_query(
            "SELECT ts, src, dst, proto, sport, dport, length, country, org, summary, is_alert FROM packets ORDER BY id",
            conn,
        )
        df_a = pd.read_sql_query(
            "SELECT ts, src, dst, proto, attack_type as attack, score, detail FROM alerts ORDER BY id",
            conn,
        )
...
def export_packets_csv(
    save_path: str = "", packet_rows: list[dict] | None = None
) -> str:
    path = save_path or str(
        LOG_DIR / f"packets_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
    ...
    df = pd.read_sql_query(
        "SELECT id, ts, src, dst, proto, sport, dport, length, country, org, summary, is_alert FROM packets ORDER BY id",
        conn,
    )
    if df.empty:
        return ""
...
def export_packets_excel(
    save_path: str = "", packet_rows: list[dict] | None = None
) -> str:
    path = save_path or str(
        LOG_DIR / f"packets_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    )
    if packet_rows is not None:
...
def export_alerts_pdf(save_path: str = "", alert_rows: list[dict] | None = None) -> str:
    if FPDF is None:
        return ""
    path = save_path or str(
        LOG_DIR / f"alerts_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    )
    ...
    if df is None:
        ...
        if conn is None:
            return ""
        df = pd.read_sql_query(
            "SELECT id, ts, src, dst, proto, attack_type as attack, score, detail FROM alerts ORDER BY id",
            conn,
        )
    if df.empty:
        return ""
    ...
    pdf.cell(
        0, 6, f"Generated: {datetime.now().isoformat(timespec='seconds')}", ln=True
    )
    pdf.ln(3)
...
def export_full_html_report(
    ...
    path = save_path or str(
        LOG_DIR / f"report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
    )
    if packet_rows is not None or alert_rows is not None or traceroute_rows is not None:
        _write_full_html(
            path,
            packet_rows_to_df(packet_rows or []),
            alert_rows_to_df(alert_rows or []),
            traceroute_rows_to_df(traceroute_rows or []),
            source="session",
        )
        return path
    if not is_persist_enabled():
    ...
    if conn is None:
        return ""
    df_p = pd.read_sql_query(
        "SELECT ts, src, dst, proto, sport, dport, length, summary FROM packets ORDER BY id",
        conn,
    )
    df_a = pd.read_sql_query(
        "SELECT ts, src, dst, proto, attack_type as attack, score, detail FROM alerts ORDER BY id",
