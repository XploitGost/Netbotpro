...
class LocalTokenSecurityTests(unittest.TestCase):
    ...
    def test_status_reports_when_local_token_is_enabled(self):
        with (
            patch.dict(
                os.environ, {"NETBOT_LOCAL_TOKEN": "desktop-secret"}, clear=False
            ),
            patch.object(main, "_observability_snapshot", return_value={}),
            patch.object(
                main.sniffer_service, "get_state", return_value={"running": False}
            ),
        ):
    ...
    def test_require_local_token_accepts_expected_header(self):
        with patch.dict(
            ...
            result = main.require_local_token(
                self._build_request({"X-NetBot-Token": "desktop-secret"})
            )
    ...
    def test_monitoring_metrics_route_declares_local_token_dependency(self):
        route = self._route("/api/monitoring/metrics")
        dependencies = {dependency.call for dependency in route.dependant.dependencies}
        self.assertIn(main.require_local_token, dependencies)
